<x-app-layout>
    <x-slot name="title">
        {{ __('Edit Variant') }} | {{ config('app.name') }}
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('variants.update', ['product' => $product->id, 'variant' => $variant->id]) }}">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <x-input id="name" class="block w-full" type="text" name="name" :value="old('name', $variant->name)" required />
                                @error('name')
                            ...
                            <div>
                                ...
                                <x-input id="default_price" class="block w-full" type="number" step="0.01" min="0" name="default_price" :value="old('default_price', $variant->default_price)" required />
                                @error('default_price')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
