    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-4">
                <a href="{{ route('transactions.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-amber-500 to-amber-600 rounded-lg text-white font-medium shadow-md hover:from-amber-600 hover:to-amber-700 transition-all">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-xl shadow-sm overflow-hidden border border-green-100">
                    <div class="p-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm font-medium text-green-800">Last Month Revenue</p>
                                <p class="text-2xl font-bold mt-1 text-gray-900">৳{{ number_format($myLastMonthRevenue, 2) }}</p>
                                <div class="flex items-center mt-2">
                                    <span class="text-sm {{ $myRevenueGrowth >= 0 ? 'text-green-600' : 'text-red-600' }}">
                                        {{ $myRevenueGrowth >= 0 ? '↑' : '↓' }} {{ abs($myRevenueGrowth) }}%
                                    </span>
                                    <span class="text-xs text-gray-500 ml-2">vs this month (৳{{ number_format($myThisMonthRevenue, 2) }})</span>
                                </div>
                            </div>
                ...
                <div class="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl shadow-sm overflow-hidden border border-purple-100">
                    <div class="p-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm font-medium text-purple-800">Last Month Payments</p>
                                <p class="text-2xl font-bold mt-1 text-gray-900">৳{{ number_format($myLastMonthPayments, 2) }}</p>
                                <div class="flex items-center mt-2">
                                    <span class="text-sm {{ $myPaymentGrowth >= 0 ? 'text-green-600' : 'text-red-600' }}">
                                        {{ $myPaymentGrowth >= 0 ? '↑' : '↓' }} {{ abs($myPaymentGrowth) }}%
                                    </span>
                                    <span class="text-xs text-gray-500 ml-2">vs this month (৳{{ number_format($myThisMonthPayments, 2) }}) • ৳{{ number_format($myOutstandingPayments, 2) }} due</span>
                                </div>
                            </div>
                                    ...
                                    <div class="ml-4 flex-1">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <span class="text-xs font-medium bg-green-100 text-green-800 px-2 py-0.5 rounded-full">
                                                {{ $product->sold_quantity_last_month }} unit last month vs </br> {{ $product->sold_quantity_this_month }} unit this month
                                            </span>
                                        </div>
