        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow rounded-lg overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                    <h3 class="text-lg font-medium text-gray-900">{{ __('Customers') }}</h3>
                    <div class="text-sm text-gray-500">
                        {{ $customers->total() }} {{ __('customers found') }}
                    </div>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        @forelse($customers as $customer)
                            <tr class="{{ $customer->trashed() ? 'bg-red-50' : '' }}">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div
                                            class="flex-shrink-0 h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center">
                                            <span
                                                class="text-indigo-600 font-medium">{{ strtoupper(substr($customer->name, 0, 1)) }}</span>
                                        </div>
                                        <div class="ml-4">
                                        ...
                                        @can('delete', $customer)
                                            ...
                                            @else
                                                <form action="{{ route('customers.destroy', $customer->id) }}"
                                                    ...
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="text-red-600 hover:text-red-900"
                                                            title="{{ __('Delete') }}">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                                                             fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke="round" stroke-width="2"
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                deleteForms.forEach(form => {
                    form.addEventListener('submit', function (e) {
                        ...
                        const title = isRestore ? 'Restore Customer' : 'Are you sure?';
                        const text = isRestore ? 'Are you sure you want to restore this customer?' : "You won't be able to revert this!";
