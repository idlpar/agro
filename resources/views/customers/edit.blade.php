        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <nav class="flex items-center space-x-2 mt-1">
                    ...
                    <a href="{{ route('users.show', $customer->id) }}" class="text-indigo-500 hover:text-indigo-700 transition-colors text-sm">
                        {{ $customer->name }}
                    </a>
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
    ...
    <div class="py-8">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow rounded-xl">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-sm text-gray-500">{{ __('Customer') }} • Member since {{ $customer->created_at->format('M Y') }}</p>
                            </div>
                        </div>
                        <div class="flex space-x-3">
                            <a href="{{ route('users.show', $customer->id) }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-medium text-sm text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all">
                ...
                <!-- Edit Form -->
                <form method="POST" action="{{ route('customers.update', $customer->id) }}" class="px-6 py-6">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    <x-input id="name" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" type="text" name="name" :value="old('name', $customer->name)" required autofocus />
                                </div>
                                @error('name')
                            ...
                            <div>
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    <x-input id="email" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" type="email" name="email" :value="old('email', $customer->email)" />
                                </div>
                                @error('email')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="sm:col-span-2">
                                <x-label for="address" :value="__('Address')" class="block text-sm font-medium text-gray-700 mb-1" />
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    <textarea id="address" name="address" rows="3" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>{{ old('address', $customer->address) }}</textarea>
                                </div>
                                @error('address')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
